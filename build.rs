...
fn find_ultralight_lib_dir() -> PathBuf {
    ...
    if let Ok(dir) = env::var("UL_DIR") {
        ...
        if has_required_libs(&path, &required_libs) {
            println!("cargo:warning=Using Ultralight from UL_DIR: {}", path.display());
            return path;
        }
        ...
        if lib_dir.exists() && has_required_libs(&lib_dir, &required_libs) {
            println!("cargo:warning=Using Ultralight from UL_DIR/lib: {}", lib_dir.display());
            return lib_dir;
        }
        ...
        if let Some(parent) = path.parent() {
            ...
            if parent_lib.exists() && has_required_libs(&parent_lib, &required_libs) {
                println!("cargo:warning=Using Ultralight from UL_DIR parent's lib: {}", parent_lib.display());
                return parent_lib;
            }
        }
    }
    ...
    if let Ok(dir) = env::var("ULTRALIGHT_DIR") {
        ...
        // Check directly in the provided path
        if has_required_libs(&path, &required_libs) {
            println!("cargo:warning=Using Ultralight from ULTRALIGHT_DIR: {}", path.display());
            return path;
        }
        ...
        let lib_dir = path.join("lib");
        if lib_dir.exists() && has_required_libs(&lib_dir, &required_libs) {
            println!("cargo:warning=Using Ultralight from ULTRALIGHT_DIR/lib: {}", lib_dir.display());
            return lib_dir;
        }
    }
    ...
    for dir in common_dirs.iter() {
        ...
        if path.exists() && has_required_libs(&path, &required_libs) {
            println!("cargo:warning=Found Ultralight libraries at: {}", path.display());
            return path;
        }
    }
    ...
    for dir in &["bin", "./bin", "../bin"] {
        ...
        if let Some(parent) = path.parent() {
            ...
            if lib_dir.exists() && has_required_libs(&lib_dir, &required_libs) {
                println!("cargo:warning=Found Ultralight libraries at: {}", lib_dir.display());
                return lib_dir;
            }
        }
    }
    ...
    panic!("Could not find Ultralight libraries. Please set UL_DIR environment variable to the directory containing the Ultralight libraries.");
}
