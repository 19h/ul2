...
use std::ptr;
use crate::ul::ffi::{ULRenderer, ULConfig, ULGamepadEvent, ULGamepadAxisEvent, ULGamepadButtonEvent,
                ulCreateRenderer, ulDestroyRenderer, ulUpdate, ulRefreshDisplay, ulRender,
                ulPurgeMemory, ulLogMemoryUsage, ulStartRemoteInspectorServer,
                ulSetGamepadDetails, ulFireGamepadEvent, ulFireGamepadAxisEvent,
                ulFireGamepadButtonEvent};
use crate::ul::config::Config;
use crate::ul::session::Session;
use crate::ul::string::String;
use crate::ul::events::{GamepadEvent, GamepadAxisEvent, GamepadButtonEvent};
...
impl Renderer {
    ...
    pub fn start_remote_inspector_server(&self, address: &str, port: u16) -> bool {
        ...
        unsafe {
            ulStartRemoteInspectorServer(self.raw, c_address.as_ptr(), port)
        }
    }
