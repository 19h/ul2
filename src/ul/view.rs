...
use std::os::raw::{c_void, c_int, c_uint, c_ulonglong, c_double};
use std::ptr;
use std::ffi::c_char;
use crate::ul::ffi::{ULView, ULRenderer, ULViewConfig, ULSession, ULString, ULCursor, ULSurface, 
                ULRenderTarget, ULMessageSource, ULMessageLevel, ULIntRect, JSContextRef,
                ULKeyEvent, ULMouseEvent, ULScrollEvent, ULChangeTitleCallback, ULChangeURLCallback,
                ULChangeTooltipCallback, ULChangeCursorCallback, ULAddConsoleMessageCallback,
                ULCreateChildViewCallback, ULCreateInspectorViewCallback, ULBeginLoadingCallback,
                ULFinishLoadingCallback, ULFailLoadingCallback, ULWindowObjectReadyCallback,
                ULDOMReadyCallback, ULUpdateHistoryCallback,
                ulCreateView, ulDestroyView, ulViewGetURL, ulViewGetTitle, ulViewGetWidth,
                ulViewGetHeight, ulViewGetDisplayId, ulViewSetDisplayId, ulViewGetDeviceScale,
                ulViewSetDeviceScale, ulViewIsAccelerated, ulViewIsTransparent, ulViewIsLoading,
                ulViewGetRenderTarget, ulViewGetSurface, ulViewLoadHTML, ulViewLoadURL,
                ulViewResize, ulViewLockJSContext, ulViewUnlockJSContext, ulViewEvaluateScript,
                ulViewCanGoBack, ulViewCanGoForward, ulViewGoBack, ulViewGoForward,
                ulViewGoToHistoryOffset, ulViewReload, ulViewStop, ulViewFocus, ulViewUnfocus,
                ulViewHasFocus, ulViewHasInputFocus, ulViewFireKeyEvent, ulViewFireMouseEvent,
                ulViewFireScrollEvent, ulViewSetChangeTitleCallback, ulViewSetChangeURLCallback,
                ulViewSetChangeTooltipCallback, ulViewSetChangeCursorCallback,
                ulViewSetAddConsoleMessageCallback, ulViewSetCreateChildViewCallback,
                ulViewSetCreateInspectorViewCallback, ulViewSetBeginLoadingCallback,
                ulViewSetFinishLoadingCallback, ulViewSetFailLoadingCallback,
                ulViewSetWindowObjectReadyCallback, ulViewSetDOMReadyCallback,
                ulViewSetUpdateHistoryCallback, ulViewSetNeedsPaint, ulViewGetNeedsPaint,
...
use crate::ul::geometry::{Rect, IntRect};
...
pub trait AddConsoleMessageCallback: Send {
    fn on_add_console_message(&self, view: &View, source: MessageSource, level: MessageLevel, 
                                message: &str, line_number: u32, column_number: u32, source_id: &str);
}

/// Callback for when a child view needs to be created.
pub trait CreateChildViewCallback: Send {
    fn on_create_child_view(&self, view: &View, opener_url: &str, target_url: &str, 
                            is_popup: bool, popup_rect: IntRect) -> Option<View>;
}

/// Callback for when an inspector view needs to be created.
pub trait CreateInspectorViewCallback: Send {
    fn on_create_inspector_view(&self, view: &View, is_local: bool, inspected_url: &str) -> Option<View>;
}
...
pub trait FailLoadingCallback: Send {
    fn on_fail_loading(&self, view: &View, frame_id: u64, is_main_frame: bool, url: &str, 
                        description: &str, error_domain: &str, error_code: i32);
}
...
extern "C" fn create_inspector_view_callback<T: CreateInspectorViewCallback>(
    ...
    unsafe {
        ...
        let result = callback.on_create_inspector_view(
            &view, 
            is_local,
            &inspected_url_str
        );
...
impl<'a> Drop for LockedJSContext<'a> {
    fn drop(&mut self) {
        unsafe {
...
impl View {
    /// Create a new view.
    pub fn new(renderer: &Renderer, width: u32, height: u32, config: &ViewConfig, session: Option<&Session>) -> Self {
        unsafe {
    ...
    pub fn set_change_title_callback<T: 'static + ChangeTitleCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetChangeTitleCallback(
                self.raw,
                std::mem::transmute(change_title_callback::<T> as extern "C" fn(*mut c_void, *mut _, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_change_url_callback<T: 'static + ChangeURLCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetChangeURLCallback(
                self.raw,
                std::mem::transmute(change_url_callback::<T> as extern "C" fn(*mut c_void, *mut _, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_change_tooltip_callback<T: 'static + ChangeTooltipCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetChangeTooltipCallback(
                self.raw,
                std::mem::transmute(change_tooltip_callback::<T> as extern "C" fn(*mut c_void, *mut _, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_change_cursor_callback<T: 'static + ChangeCursorCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetChangeCursorCallback(
                self.raw,
                std::mem::transmute(change_cursor_callback::<T> as extern "C" fn(*mut c_void, *mut _, _)),
                user_data
            );
        }
    }
    ...
    pub fn set_add_console_message_callback<T: 'static + AddConsoleMessageCallback>(&self, callback: T) {
        unsafe {
            let user_data = CallbackData::new(callback);
            ulViewSetAddConsoleMessageCallback(
                self.raw,
                std::mem::transmute(add_console_message_callback::<T> as extern "C" fn(*mut c_void, *mut _, _, _, *mut _, _, _, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_create_child_view_callback<T: 'static + CreateChildViewCallback>(&self, callback: T) {
        unsafe {
            let user_data = CallbackData::new(callback);
            ulViewSetCreateChildViewCallback(
                self.raw,
                std::mem::transmute(create_child_view_callback::<T> as extern "C" fn(*mut c_void, *mut _, *mut _, *mut _, bool, _) -> *mut _),
                user_data
            );
        }
    }
    ...
    pub fn set_create_inspector_view_callback<T: 'static + CreateInspectorViewCallback>(&self, callback: T) {
        unsafe {
            let user_data = CallbackData::new(callback);
            ulViewSetCreateInspectorViewCallback(
                self.raw,
                std::mem::transmute(create_inspector_view_callback::<T> as extern "C" fn(*mut c_void, *mut _, bool, *mut _) -> *mut _),
                user_data
            );
        }
    }
    ...
    pub fn set_begin_loading_callback<T: 'static + BeginLoadingCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetBeginLoadingCallback(
                self.raw,
                std::mem::transmute(begin_loading_callback::<T> as extern "C" fn(*mut c_void, *mut _, u64, bool, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_finish_loading_callback<T: 'static + FinishLoadingCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetFinishLoadingCallback(
                self.raw,
                std::mem::transmute(finish_loading_callback::<T> as extern "C" fn(*mut c_void, *mut _, u64, bool, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_fail_loading_callback<T: 'static + FailLoadingCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetFailLoadingCallback(
                self.raw,
                std::mem::transmute(fail_loading_callback::<T> as extern "C" fn(*mut c_void, *mut _, u64, bool, *mut _, *mut _, *mut _, i32)),
                user_data
            );
        }
    }
    ...
    pub fn set_window_object_ready_callback<T: 'static + WindowObjectReadyCallback>(&self, callback: T) {
        unsafe {
            let user_data = CallbackData::new(callback);
            ulViewSetWindowObjectReadyCallback(
                self.raw,
                std::mem::transmute(window_object_ready_callback::<T> as extern "C" fn(*mut c_void, *mut _, u64, bool, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_dom_ready_callback<T: 'static + DOMReadyCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetDOMReadyCallback(
                self.raw,
                std::mem::transmute(dom_ready_callback::<T> as extern "C" fn(*mut c_void, *mut _, u64, bool, *mut _)),
                user_data
            );
        }
    }
    ...
    pub fn set_update_history_callback<T: 'static + UpdateHistoryCallback>(&self, callback: T) {
        unsafe {
            ...
            ulViewSetUpdateHistoryCallback(
                self.raw,
                std::mem::transmute(update_history_callback::<T> as extern "C" fn(*mut c_void, *mut _)),
                user_data
            );
        }
    }
